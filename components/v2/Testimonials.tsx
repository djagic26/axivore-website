...
const TESTI_ACCENTS = [
  { rgb: "124,92,255", avatar: "linear-gradient(135deg, #7C5CFF, #A09AFF)", border: "rgba(124,92,255,0.34)", result: "rgba(124,92,255,0.1)", resultColor: "#A09AFF", resultBorder: "rgba(124,92,255,0.2)" },
  { rgb: "255,61,197", avatar: "linear-gradient(135deg, #FF3DC5, #ff8de8)", border: "rgba(255,61,197,0.3)",  result: "rgba(255,61,197,0.08)", resultColor: "#FF3DC5", resultBorder: "rgba(255,61,197,0.18)" },
  { rgb: "91,138,255",  avatar: "linear-gradient(135deg, #5B8AFF, #93baff)", border: "rgba(91,138,255,0.3)",  result: "rgba(91,138,255,0.08)", resultColor: "#5B8AFF", resultBorder: "rgba(91,138,255,0.18)" },
];
        ...
        <div className="text-center mb-20 relative">
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-center pointer-events-none overflow-hidden select-none" style={{ zIndex: 0 }}>
            <span className="font-black uppercase tracking-[-0.04em] whitespace-nowrap"
              style={{ fontSize: "clamp(50px,10vw,160px)", color: isDark ? "rgba(255,255,255,0.1)" : "rgba(124,92,255,0.1)" }}>
              TESTIMONIALS
            </span>
          </div>
          ...
          {t.testimonials.items.map((item, i) => {
            ...
            return (
              <motion.div key={i}
                ...
                style={{
                  background: isDark
                    ? `linear-gradient(145deg, rgba(${acc.rgb},0.14) 0%, rgba(6,3,14,0.97) 70%)`
                    : "rgba(255,255,255,0.88)",
                  border: `1px solid ${isDark ? acc.border : "rgba(124,92,255,0.15)"}`,
                  backdropFilter: isDark ? "none" : "blur(20px)",
                  boxShadow: isDark ? `0 6px 44px rgba(${acc.rgb},0.18)` : "0 4px 24px rgba(100,60,220,0.08)",
                }}>
