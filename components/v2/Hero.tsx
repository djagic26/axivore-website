...
export function Hero() {
  ...
  const heroBg = isDark
    ? "#120d08"
    : "linear-gradient(145deg, #FBF2E4 0%, #F8ECDA 20%, #FCF5E9 50%, #FEFBF6 80%, #FFFFFF 100%)";
  ...
  return (
    <section className="relative min-h-screen flex flex-col pt-16 overflow-hidden"
      ...
      {isDark && (
        <>
          <div className="absolute pointer-events-none inset-x-0 top-0"
            style={{ height: "85%",
              background: "radial-gradient(ellipse 38% 60% at 50% -8%, rgba(211, 120, 47,0.95) 0%, rgba(170, 104, 49,0.55) 18%, rgba(108, 72, 40,0.22) 38%, rgba(52, 33, 16,0.06) 58%, transparent 72%)" }} />
          <div className="absolute pointer-events-none inset-x-0 top-0"
            style={{ height: "50%",
              background: "radial-gradient(ellipse 18% 35% at 50% -2%, rgba(223, 157, 105,0.9) 0%, rgba(216, 137, 72,0.5) 20%, transparent 55%)" }} />
          <motion.div className="absolute pointer-events-none"
            animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0.85, 0.5] }}
            transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
            style={{ width: 550, height: 550, top: "10%", right: "-6%",
              background: "radial-gradient(ellipse, rgba(181,80,46,0.28) 0%, rgba(188, 99, 40,0.1) 40%, transparent 65%)",
              filter: "blur(60px)" }} />
          <motion.div className="absolute pointer-events-none"
            animate={{ scale: [1, 1.15, 1], opacity: [0.4, 0.75, 0.4] }}
            transition={{ duration: 13, repeat: Infinity, ease: "easeInOut", delay: 4 }}
            style={{ width: 500, height: 450, top: "20%", left: "-8%",
              background: "radial-gradient(ellipse, rgba(70,130,255,0.3) 0%, rgba(140, 101, 53,0.12) 45%, transparent 68%)",
              filter: "blur(60px)" }} />
          <motion.div className="absolute pointer-events-none"
            animate={{ opacity: [0.4, 0.7, 0.4] }}
            transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 2 }}
            style={{ width: 800, height: 300, bottom: 60, left: "50%", transform: "translateX(-50%)",
              background: "radial-gradient(ellipse, rgba(188, 108, 40,0.18) 0%, rgba(181,80,46,0.07) 40%, transparent 65%)",
              filter: "blur(40px)" }} />
        </>
      )}
      {!isDark && (
        <>
          <motion.div
            ...
            style={{ width: 950, height: 850, top: "-28%", left: "12%",
              background: "radial-gradient(ellipse, rgba(216, 139, 72,0.14) 0%, rgba(181, 113, 53,0.06) 40%, transparent 65%)",
              filter: "blur(75px)" }} />
          <motion.div
            ...
            style={{ width: 700, height: 700, top: "0%", right: "-10%",
              background: "radial-gradient(ellipse, rgba(213, 121, 56,0.1) 0%, transparent 60%)",
              filter: "blur(68px)" }} />
          <motion.div
            ...
            style={{ width: 520, height: 520, bottom: "18%", left: "-6%",
              background: "radial-gradient(ellipse, rgba(225, 164, 113,0.11) 0%, transparent 60%)",
              filter: "blur(62px)" }} />
        </>
      )}
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{
          backgroundImage: `radial-gradient(circle, ${isDark ? "rgba(224,163,96,0.18)" : "rgba(118, 85, 55,0.14)"} 1px, transparent 1px)`,
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[45%] pointer-events-none"
        style={{ background: `linear-gradient(to bottom, transparent, ${isDark ? "#120d08" : "#FDFAF4"}f5)` }} />
      <div className="absolute inset-y-0 left-0 w-[15%] pointer-events-none"
        style={{ background: `linear-gradient(to right, ${isDark ? "#120d08" : "#FDFAF4"}, transparent)` }} />
      <div className="absolute inset-y-0 right-0 w-[15%] pointer-events-none"
        style={{ background: `linear-gradient(to left, ${isDark ? "#120d08" : "#FDFAF4"}, transparent)` }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center min-h-[calc(100vh-64px)] py-20 md:py-24">
          <div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              style={{
                background: isDark ? "rgba(201,124,60,0.1)" : "rgba(201,124,60,0.08)",
                border: `1px solid ${isDark ? "rgba(201,124,60,0.25)" : "rgba(201,124,60,0.2)"}`,
              }}>
              <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#E0A360" }} />
              <span className="text-[10.5px] tracking-[0.18em] uppercase font-medium" style={{ color: "#E0A360" }}>
                {t.hero.badge}
              </span>
            </motion.div>

            {/* Each line is a separate reveal block with no whitespace between
                them in the DOM, so screen readers / copy-paste would read
                "...worksEven at night" run together — aria-label carries the
                real sentence, the visual spans are hidden from assistive tech. */}
            <h1 className="mb-8" aria-label={`${heroLines.join(" ")}.`}>
                ...
                <div key={i} className="overflow-hidden">
                  <motion.div initial={{ y: "105%" }} animate={{ y: 0 }}
                    ...
                    {i === heroLines.length - 1 ? (
                      <>{line}<span style={{ ...(isDark ? { background: "none", WebkitTextFillColor: "#C97C3C" } : { color: "#C97C3C" }) }}>.</span></>
                    ) : line}
                  </motion.div>
                </div>
              ))}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <motion.a href="https://calendly.com/hello-axivore/kostenloses-gesprach"
                ...
                whileHover={{ scale: 1.04, boxShadow: "0 10px 50px rgba(188, 102, 40,0.65)" }}
                whileTap={{ scale: 0.97 }}
                ...
                style={{ background: "linear-gradient(135deg, #C97C3C 0%, #C97C3C 40%, #d9a54e 100%)", boxShadow: "0 6px 36px rgba(201,124,60,0.5)", transition: "box-shadow 0.25s" }}>
                <motion.div
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <a href="mailto:hello@axivore.io"
                ...
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#E0A360" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }}>
              <p className="text-[9.5px] tracking-[0.24em] uppercase mb-4"
                style={{ color: isDark ? "rgba(255,255,255,0.42)" : "rgba(118, 85, 55,0.45)" }}>
                {t.hero.socialProof}
              </p>
              <div className="flex items-center gap-7 flex-wrap">
                {["Zapier", "OpenAI", "Claude", "Supabase", "Next.js", "Stripe"].map((name) => (
                  <span key={name} className="text-[13px] font-semibold"
                    style={{ color: isDark ? "rgba(255,255,255,0.52)" : "rgba(67, 46, 26,0.38)" }}>
                    {name}
                  </span>
                ))}
              </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <div className="absolute right-[-8px] bottom-14 text-right pr-2">
              <p className="text-[10.5px] leading-[1.9]"
                style={{ color: isDark ? "rgba(255,255,255,0.48)" : "rgba(67, 46, 26,0.55)" }}>
              ...
              <div className="w-1 h-1 rounded-full ml-auto mt-2"
                style={{ background: "#E0A360", boxShadow: "0 0 8px rgba(224,163,96,0.8)" }} />
            </div>
          </motion.div>
        </div>
      </div>
              ...
              {["AXIVORE", ...t.hero.marqueeWords].map((word, j) => (
                <span key={j} className="font-bold uppercase tracking-[-0.02em] mx-8"
                  style={{
                    ...
                    color: "transparent",
                    WebkitTextStroke: isDark ? "1.5px #C97C3C" : "1.5px rgba(201,124,60,0.5)",
                    filter: isDark
                      ? "drop-shadow(0 0 6px #C97C3C) drop-shadow(0 0 18px #C97C3C80)"
                      : "none",
                  }}>
                  {word}
                  <span style={{
                    color: "transparent",
                    WebkitTextStroke: isDark ? "1.5px #E0A360" : "1.5px rgba(201,124,60,0.5)",
                    filter: isDark ? "drop-shadow(0 0 8px #E0A360)" : "none",
                  }}> ·</span>
                </span>
              ))}
            </div>
