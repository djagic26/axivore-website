...
const VALUE_ICONS = ["→", "◎", "▲"];
const VALUE_RGBS = ["124,92,255", "255,61,197", "91,138,255"];
const VALUE_COLORS = ["#A09AFF", "#FF3DC5", "#5B8AFF"];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative overflow-hidden mb-8">
              <div className="absolute inset-y-0 left-0 flex items-center pointer-events-none select-none" style={{ zIndex: 0 }}>
                <span className="font-black uppercase tracking-[-0.04em] whitespace-nowrap"
                  style={{ fontSize: "clamp(70px,12vw,160px)", color: isDark ? "rgba(255,255,255,0.1)" : "rgba(124,92,255,0.1)" }}>
                  STUDIO
                </span>
              </div>
              <div className="relative" style={{ zIndex: 1 }}>
                ...
                <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  ...
                  {t.about.headline}<span style={{ color: "#7C5CFF" }}>.</span>
                </motion.h2>
              </div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              ...
              {t.about.badges.map((word) => (
                <span key={word} className="text-[12px] font-bold px-4 py-1.5 rounded-full"
                  style={{ background: isDark ? "rgba(124,92,255,0.12)" : "rgba(124,92,255,0.08)", border: "1px solid rgba(124,92,255,0.28)", color: "#A09AFF" }}>
                  {word}
                </span>
              ))}
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <div className="rounded-[28px] overflow-hidden relative"
              style={{
                background: isDark
                  ? "linear-gradient(145deg, rgba(124,92,255,0.14) 0%, rgba(6,3,14,0.97) 70%)"
                  : "linear-gradient(145deg, rgba(124,92,255,0.06) 0%, rgba(245,240,255,0.95) 100%)",
                border: `1px solid rgba(124,92,255,${isDark ? "0.28" : "0.22"})`,
                boxShadow: isDark ? "0 8px 60px rgba(124,92,255,0.2)" : "0 8px 60px rgba(124,92,255,0.12)",
              }}>
              <div className="h-[3px]" style={{ background: "linear-gradient(90deg, transparent, rgba(124,92,255,0.85), transparent)" }} />
              <div className="absolute top-0 right-0 w-64 h-64 pointer-events-none"
                style={{ background: "radial-gradient(ellipse, rgba(124,92,255,0.18) 0%, transparent 65%)", filter: "blur(32px)", transform: "translate(30%,-30%)" }} />

              <div className="p-7 flex items-center gap-5 relative"
                style={{ borderBottom: `1px solid ${isDark ? "rgba(255,255,255,0.07)" : "rgba(124,92,255,0.1)"}` }}>
                <div className="w-[72px] h-[72px] rounded-2xl overflow-hidden flex-shrink-0"
                  style={{ border: "2px solid rgba(124,92,255,0.38)", boxShadow: "0 4px 20px rgba(124,92,255,0.3)" }}>
                ...
                <div>
                  ...
                  <div className="text-[12px] font-semibold" style={{ color: "#A09AFF" }}>Gründer & KI-Architekt</div>
